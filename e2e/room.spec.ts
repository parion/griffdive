...
test('two divers sync one dive; late joiner gets the snapshot', async ({ browser }) => {
  ...
  await expect(pageB.getByText('Awaiting host', { exact: true })).toBeVisible()
  ...
  await expect(pageB.getByText('Strain call — awaiting host')).toBeVisible()
  await pageA.getByRole('button', { name: 'Decline' }).click()
  await expect(pageB.getByText('Standard forces — no strain')).toBeVisible()
